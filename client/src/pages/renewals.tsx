...
export default function RenewalsPage() {
  ...
  const loadRenewals = async () => {
    setLoading(true)
    try {
      const { getRenewals } = await import("@/lib/actions/renewal-actions")
  ...
  const handleRemoveDuplicates = async () => {
    setLoading(true)
    try {
      const { removeDuplicateRenewals } = await import("@/lib/actions/renewal-actions")
  ...
  const handleBulkDelete = async () => {
    setLoading(true)
    try {
      const { bulkDeleteRenewals } = await import("@/lib/actions/renewal-actions")
  ...
  const handleBulkStatusChange = async () => {
    setLoading(true)
    try {
      const { bulkUpdateRenewalStatus } = await import("@/lib/actions/renewal-actions")
  ...
  const handleBulkArchive = async () => {
    setLoading(true)
    try {
      const { bulkArchiveRenewals } = await import("@/lib/actions/renewal-actions")
  ...
  const handleArchive = async (renewalId: string) => {
    try {
      const { archiveRenewal } = await import("@/lib/actions/renewal-actions")
  ...
  const handleView = (renewalId: string) => {
    navigate(`/renewals/${renewalId}`)
  }

  const handleEdit = (renewalId: string) => {
    navigate(`/renewals/${renewalId}/edit`)
  }
  ...
  const handleDelete = async (renewalId: string) => {
    ...
    setLoading(true)
    try {
      const { deleteRenewal } = await import("@/lib/actions/renewal-actions")
  ...
  const handleStatusChange = async (renewalId: string, newStatus: string) => {
    setLoading(true)
    try {
      const { updateRenewalStatus } = await import("@/lib/actions/renewal-actions")
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            ...
            <Button className="bg-white text-blue-600 hover:bg-gray-100" onClick={() => navigate("/renewals/new")}>
