...
// Hardcoded credentials for this import
const supabaseUrl = "https://nybxlheqpgktxpwkuigg.supabase.co"
...
console.log("🚀 Starting renewal import with hardcoded credentials...")
...
function mapRenewalData(csvData) {
  ...
  return csvData.map((row, index) => {
    const renewal = {
      ...
      // Map to actual database columns only
      ...
      expiration_date: parseDate(row["Expiration Date"])
        ? new Date(parseDate(row["Expiration Date"]) + "T00:00:00Z").toISOString()
        : null,
      ...
      retail_agency_name: row["Location"] || null,
