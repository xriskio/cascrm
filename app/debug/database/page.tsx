    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Check what data is actually in your database tables</p>
        </div>
          ...
          <Card key={tableName}>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <span className={`text-lg font-bold ${tableData.count > 0 ? "text-green-600" : "text-gray-400"}`}>
                  {tableData.count}
                </span>
              </CardTitle>
            </CardHeader>
            <CardContent>
              {tableData.error ? (
                ...
                <div className="space-y-3">
                  <div>
                    <div className="text-sm font-medium text-gray-700">Records: {tableData.count}</div>
                    <div className="text-sm text-gray-500">Columns: {tableData.columns?.length || 0}</div>
                  </div>

                  {tableData.columns && tableData.columns.length > 0 && (
                    <div>
                      <div className="text-sm font-medium text-gray-700 mb-1">Columns:</div>
                      <div className="text-xs text-gray-600 space-y-1">
                        {tableData.columns.slice(0, 8).map((col: string) => (
                          <div key={col} className="bg-gray-100 px-2 py-1 rounded text-xs">
                            {col}
                          </div>
                        ))}
                        {tableData.columns.length > 8 && (
                          <div className="text-gray-400">... and {tableData.columns.length - 8} more</div>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  {tableData.sample && tableData.sample.length > 0 && (
                    <div>
                      <div className="text-sm font-medium text-gray-700 mb-1">Sample Data:</div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div className="text-sm text-gray-600">Tables Checked</div>
            </div>
          </div>
        </CardContent>
      </Card>
