...
import { getInspections, getInspectionStats } from "@/app/actions/inspection-actions"
...
export default function InspectionsPage() {
  ...
  const handleScheduleInspection = () => {
    router.push("/inspections/schedule")
  }

  const handleViewRequests = () => {
    router.push("/inspections/requests")
  }

  const handleViewCompliance = () => {
    router.push("/inspections/compliance")
  }
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { bg: string; text: string; label: string }> = {
      pending: { bg: "from-yellow-100 to-orange-100", text: "text-yellow-300", label: "Scheduled" },
      confirmed: { bg: "from-green-100 to-emerald-100", text: "text-green-300", label: "Confirmed" },
      completed: { bg: "from-blue-100 to-cyan-100", text: "text-blue-300", label: "Completed" },
      compliance_issue: { bg: "from-red-100 to-pink-100", text: "text-red-300", label: "Issues Found" },
    }
      ...
      <div className="container mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div
            ...
            className="group bg-card backdrop-blur-sm border border-border/50 rounded-2xl p-8 hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer"
          >
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-xl font-semibold ml-4 text-foreground">Inspection Requests</h2>
            </div>
          ...
          >
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-xl font-semibold ml-4 text-foreground">Pending Compliance</h2>
            </div>
            <p className="text-muted-foreground mb-4">Track inspections with missing photos, documents, or certificates.</p>
          ...
          >
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-xl font-semibold ml-4 text-foreground">Schedule Inspection</h2>
            </div>
            <p className="text-muted-foreground mb-4">
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white/30 divide-y divide-border/50">
                  {upcomingInspections.map((inspection) => (
                    <tr key={inspection.id} className="hover:bg-card transition-colors duration-200">
                      <td className="px-6 py-4 text-foreground font-mono text-sm">{inspection.inspection_number}</td>
                      <td className="px-6 py-4 text-foreground font-medium">{inspection.named_insured}</td>
                      <td className="px-6 py-4 text-muted-foreground">{inspection.policy_number}</td>
                      <td className="px-6 py-4 text-muted-foreground">{inspection.policy_type}</td>
                      <td className="px-6 py-4 text-muted-foreground">{formatDate(inspection.effective_date)}</td>
                      <td className="px-6 py-4 text-muted-foreground">{inspection.contact_name}</td>
              ...
              <div className="text-center py-12">
                ...
                <button
                  onClick={handleScheduleNow}
                  className="mt-4 text-blue-600 hover:text-blue-400 font-medium"
                >
                ...
                <tbody className="bg-white/30 divide-y divide-border/50">
                  {completedInspections.map((inspection) => (
                    <tr key={inspection.id} className="hover:bg-card transition-colors duration-200">
                      ...
                      <td className="px-6 py-4 text-muted-foreground">{inspection.policy_number}</td>
                      <td className="px-6 py-4 text-muted-foreground">{inspection.policy_type}</td>
                      <td className="px-6 py-4 text-muted-foreground">{formatDate(inspection.created_at)}</td>
                      <td className="px-6 py-4 text-muted-foreground">{inspection.contact_name}</td>
                      <td className="px-6 py-4">{getStatusBadge(inspection.status)}</td>
                      <td className="px-6 py-4">
                        <div className="flex gap-2">
