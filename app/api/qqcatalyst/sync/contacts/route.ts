...
export async function POST(req: NextRequest) {
  ...
  while (pg <= 50) {
    ...
    if (!r.ok) { const t = await r.text(); return NextResponse.json({ success: false, error: 'QQ ' + r.status + ' contacts: ' + t.slice(0, 300) }, { status: 500 }) }
  ...
  await sb.from('qqcatalyst_contacts').upsert(
    all.map((c: any) => ({
      entity_id: String(c.EntityID || c.ContactID || c.ID || ''),
  ...
  const lr = all.map((c: any) => ({
    email: c.Email || c.PrimaryEmail || null,
    phone: c.Phone || c.PrimaryPhone || null,
    contact_name: (c.FullName || c.Name || ((c.FirstName || '') + ' ' + (c.LastName || '')).trim()) || 'Unknown',
  ...
  const cr = all.map((c: any) => ({
    qq_catalyst_id: String(c.EntityID || c.ContactID || c.ID || ''),
    contact_name: c.FullName || c.Name || null,
    first_name: c.FirstName || null,
    last_name: c.LastName || null,
    email: c.Email || c.PrimaryEmail || null,
    phone: c.Phone || c.PrimaryPhone || null,
