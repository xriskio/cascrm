...
export default function SupplementalsPage() {
  ...
  useEffect(() => {
    async function fetchResources() {
      try {
        ...
        console.log("Fetching supplemental resources")

        const { data, error } = await supabase
          .from("agency_resources")
          .select("*")
          .eq("category", "Supplementals")
          .order("created_at", { ascending: false })

        if (error) {
          console.error("Error fetching supplemental resources:", error)
          throw error
        }

        console.log("Fetched supplemental resources:", data)
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Supplementals</h1>
